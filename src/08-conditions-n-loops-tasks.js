...
function findFirstSingleChar(str) {
  return str.split('').find((item, _, array) => array.indexOf(item) === array.lastIndexOf(item)) || null;
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = str.split('').reverse().reduce((acc, item, index) => {
    let num = +item;
    if (index % 2) {
      num *= 2;
      if (num > 9) {
...
// return winner ('X' or '0') or undefined if no winner
  const winner = (arr) => {
